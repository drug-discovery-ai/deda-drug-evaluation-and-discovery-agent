...
class TestBioinformaticsChatClient:
    ...
    def chat_client(self, mock_agent_executor, mock_create_agent, mock_create_tools, mock_chat_openai, mock_env_vars, mock_clients):
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("clients,expected_tools_call", [
        (("custom", "custom", "custom"), None),  # Custom clients
        ((None, None, None), {"uniprot_client": None, "pdb_client": None, "sequence_analyzer": None}),  # Default clients
    ])
    @patch('drug_discovery_agent.chat.ChatOpenAI')
    @patch('drug_discovery_agent.chat.create_bioinformatics_tools')
    @patch('drug_discovery_agent.chat.create_openai_tools_agent')
    @patch('drug_discovery_agent.chat.AgentExecutor')
    def test_initialization(self, mock_agent_executor, mock_create_agent, mock_create_tools, mock_chat_openai, 
                          clients, expected_tools_call, mock_env_vars, mock_clients):
    ...
    async def test_chat_success(self, chat_client, spike_protein_uniprot_id):
        ...
        expected_response = f"Here's information about the protein {spike_protein_uniprot_id}..."
        chat_client._mock_executor.ainvoke.return_value = {"output": expected_response}
        
        result = await chat_client.chat(f"Tell me about protein {spike_protein_uniprot_id}")
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("command,expected_result,mock_method", [
    ...
    async def test_chat_loop_error_handling(self, chat_client, exception, expected_message):
    ...
    async def test_chat_loop_regular_chat(self, chat_client, spike_protein_uniprot_id):
        ...
        with patch('builtins.input', side_effect=inputs):
            with patch('builtins.print') as mock_print:
                chat_client._mock_executor.ainvoke.return_value = {"output": "Test response"}
...
class TestMainFunctions:
    ...
    @patch('drug_discovery_agent.chat.BioinformaticsChatClient')
    async def test_async_main_errors(self, mock_client_class, mock_print, exception, expected_message):
